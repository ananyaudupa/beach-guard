...
# Sidebar Navigation
st.sidebar.title("🏖️ Beach Guard AI")
page = st.sidebar.radio("Navigation", ["🏠 Home", "⚙️ Admin Panel"])

# ========================
# Home Page
# ========================
if page == "🏠 Home":
    st.markdown('<div class="main-header"><h1>🏖️ Beach Safety Monitoring System</h1></div>', 
                unsafe_allow_html=True)
    
    # Display current settings (read-only for non-admin)
    col1, col2 = st.columns(2)
    with col1:
        st.info(f"**Safe Distance:** {st.session_state.safe_distance_px} px")
    with col2:
        st.info(f"**Safe Threshold:** {st.session_state.safe_threshold} px")
                            ...
                            while True:
                                ...
                                if frame_stats["danger"] > 0:
                                    if current_alert != "danger":
                                        send_telegram_message("🚨 DANGER ALERT: People too close to water!")
...
# ========================
    ...
    if not st.session_state.authenticated:
        ...
        st.info(f"""
        - **Safe Distance:** {st.session_state.safe_distance_px} px
        - **Safe Threshold:** {st.session_state.safe_threshold} px
        - **Moderate Threshold:** {st.session_state.safe_distance_px} px (auto-calculated)
        """)
...
# Footer
st.sidebar.markdown("---")
st.sidebar.markdown("**Beach Guard AI** v1.0")
st.sidebar.markdown("Real-time beach safety monitoring")